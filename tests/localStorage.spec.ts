...
test.describe('localStorage persistence for owner/repo form', () => {
  ...
  test('should save owner and repo values to localStorage as user types', async ({ page }) => {
    await page.goto('/');
    ...
    const storedOwner = await page.evaluate(() => localStorage.getItem('github_owner'));
    const storedRepo = await page.evaluate(() => localStorage.getItem('github_repo'));
    
    expect(storedOwner).toBe(testOwner);
    expect(storedRepo).toBe(testRepo);
  });

  test('should restore values from localStorage after page reload', async ({ page }) => {
    await page.goto('/');
  ...
  test('should update localStorage when form values change', async ({ page }) => {
    await page.goto('/');
    ...
    let storedOwner = await page.evaluate(() => localStorage.getItem('github_owner'));
    let storedRepo = await page.evaluate(() => localStorage.getItem('github_repo'));
    ...
    storedOwner = await page.evaluate(() => localStorage.getItem('github_owner'));
  ...
  test('should handle empty localStorage gracefully on initial load', async ({ page }) => {
    await page.goto('/');
  ...
  test('should pre-populate form with existing localStorage values', async ({ page }) => {
    await page.goto('/');
  ...
  test('should persist values when typing character by character', async ({ page }) => {
    await page.goto('/');
  ...
  test('should maintain localStorage values across browser sessions simulation', async ({ page }) => {
    await page.goto('/');
